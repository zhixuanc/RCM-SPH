...
\\[3pt]
%need to show that equal density can help reduce oscillation.
%For compressible flow, smoothing lengths. Unfortunately the original manuscript did not delivery this point clearly. The shock tube tests were intentionally designed to make such oscillation more obvious by assigning different particles masses. In all shock tube tests presented in previous work, uniform particle mass is adopted while in our manuscript, different particle mass are assigned for particles on right side and left side. Different particle mass will amplify the oscillation and help to demonstrate the merit of present method in a more obvious manner.
...
%Some questions/confusion are general question for grid-based CFD. We will try to give detailed response. But we will not include these details in our revised manuscript to avoid being too disruptive to readers. 
...
%We understand that the reviewer has solid background in SPH but relatively less background in mesh based CFD (computational fluid dynamics) methods.
