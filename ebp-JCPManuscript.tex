...
\begin{document}
\begin{frontmatter}
\title{IA Random Choice SPH Scheme with Adaptive Viscosity}
...
To summarize, our RSPH alogrithm proceeds as follows:
\begin{itemize}
...
\section{Conclusion}
We proposed RSPH, a novel SPH scheme for solving hyperbolic non-linear PDEs that combines the Random Choice Method with
...
On the other hand, RSPH does not redress other methodological questions that must be decided in order to use SPH, 
...
This may be particularly important when applying RSHP to other systems, where the explicit construction on which the HLLC solver
